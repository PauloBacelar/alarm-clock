...
function showErrorMessage() {
  result.innerHTML =
    "You're trying to set an alarm with invalid inputs, or alarm is already set.<br/> Refresh the page and try again";
}
  ...
  if (isExactTime) {
    ...
    if (currentHour > hours) {
      return (
        hours * 3600000 +
        minutes * 60000 +
        ((23 - currentHour) * 3600000 + (59 - currentMinutes) * 60000)
      );
      ...
      return (
        (hours - currentHour) * 3600000 + (minutes - currentMinutes) * 60000
      );
    } else {
      if (currentMinutes > minutes) {
...
button.addEventListener("click", function () {
  ...
  let [hoursItWillPlay, minutesItWillPlay] = calcWhenWillPlay(
    hours,
    minutes,
    exactTime.checked
  );
